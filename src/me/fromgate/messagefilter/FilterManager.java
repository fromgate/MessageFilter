...
public class FilterManager {
	...
	public static void load(){
		...
		for (File f : dir.listFiles()){
			...
			for (String key : cfg.getKeys(false)){
				...
				Filter filter = new Filter (type, f.getName().replace(".yml", ""), messageMask, replaceMask, cooldownTime);
	...
	public static String processMessage (Player player, String message){
		...
		for (String key : filters.keySet()){
			Filter filter = filters.get(key);
			newMessage = filter.processMessage(player, key, message);
			if (!newMessage.equals(message)) break;
		}
		return newMessage;
	}
